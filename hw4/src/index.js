...
function findAllPSiblings(where) {
    return [].filter.call(where.childNodes, (node) => {
        return node.nextSibling && node.nextSibling.nodeName == 'P';
    })
}
...
function deleteTextNodes(where) {
    [].forEach.call(where.childNodes, (node) => {
        if (node.nodeType == 3) {
...
function collectDOMStat(root) {
    var n,
        ...
        tags = {},
        classes = {},
        cls = root.querySelectorAll('[class]'),
        walkText = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, null, false),
        walkElements = document.createTreeWalker(root, NodeFilter.SHOW_ELEMENT, null, false);

    [].forEach.call(cls, (c) => {
        let h = c.className.split(' ');

        for (n in h) {
            classes[h[n]] = classes.hasOwnProperty([h[n]]) ?
                classes[h[n]] + 1 : 1;
        }
    });
    ...
    while (n = walkElements.nextNode()) {
        tags[n.nodeName] = tags.hasOwnProperty([n.nodeName]) ?
            tags[n.nodeName] + 1 : 1;
    }
        ...
        observer = new MutationObserver(function (mutations) {
            mutations.forEach(function (mutation) {
                if (mutation.addedNodes.length) {
                    fn({ type: 'insert', nodes: [].slice.call(mutation.addedNodes) });
                }

                if (mutation.removedNodes.length) {
                    fn({ type: 'remove', nodes: [].slice.call(mutation.removedNodes) });
                }
            }
            )
        }),
